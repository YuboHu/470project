...
def search(request):
    ...
    # If it's a HTTP POST, we're interested in processing form data.
		...
		events_list_wc= Events.objects.filter(starttime__lt=start).filter(starttime__gt=now).filter(title__contains=title).filter(location__contains=location).filter(age_limit__lte=age).order_by('starttime')#.order_by('engaged_people')
		#events_list=set()
